...
def run(args=None): 
    ...
    if args.mode=='live':
        ...
        cerebro.broker.set_coc(True) # Cheat on Close this is so that the order gets executed on the same bar
    ...
    df_perf=pd.DataFrame(results[0].analyzers.strat_perf.get_analysis(), index=[0])
    print(df_perf)
    df_perf.to_csv('strat_performance.csv')
    df_pos=pd.DataFrame(results[0].analyzers.pos_perf.get_analysis())
    df_pos.to_csv('position.csv')
...
def parse_args(pargs=None):
    ...
    parser.add_argument('--tickers', nargs='*' ,required=False,default=['BA'], type=str,  #['PZZA'] #['BOM500010,BOM500034,BOM500087']
                        help='Pass the tickers with space')
    ...
    parser.add_argument('--cash', default=1000000, type=float,
                        metavar='kwargs', help='kwargs in k1=v1,k2=v2 format')

    parser.add_argument('--strat_name', required=False, default='alpha_indicator_strategy', 
                        metavar='kwargs', help='kwargs in k1=v1,k2=v2 format')
    ...
    parser.add_argument('--plot', required=False, default=True, type=args_parse_other.str2bool, const=True, nargs='?',
                    help='Plot the results')

    parser.add_argument('--load_indicator_db', required=False, default=False, type=args_parse_other.str2bool, const=True, nargs='?',
